 ...
 *
 */
public class VisItSwtConnection implements VisItInitializedCallback, AttributeSubjectCallback {
	...
	public void initialized() {
		...
		client.getViewerState().registerCallback("ViewerClientInformation", this);
	}
	...
	/**! window callback */
	public void registerVisualization(VISIT_CONNECTION_TYPE type, int windowId, VisualizationUpdateCallback callback) {
		
		System.out.println("add connection");
		if(!windowCallbacks.containsKey(windowId)) {
			windowCallbacks.put(windowId, new ArrayList<VisItConnectionStruct>());
		}
	...
	public void setParameters(String userName, String instanceId,
			VISIT_CONNECTION_TYPE connType, int windowWidth, int windowHeight, int windowId) {
	...
	public void update(AttributeSubject subject) {
		...
		for (int i = 0; i < imageList.size(); ++i) {
			...
			for(int j = 0; j < structs.size(); ++j) {
				...
				structs.get(j).callback.update(VISIT_CONNECTION_TYPE.IMAGE, output);
			}
		}
